...
// This test makes sure that the PodPatcher struct contains all and only the annotations defined as constants in the annotations package.
func TestAnnotationCompletness(t *testing.T) {
	annotationsPkg := []string{}
	annotationsStruct := []string{}

	// Load the annotations defined as constants in pkg/annotations/annotations.go, whose name starts with Key*
	fset := token.NewFileSet()
	node, err := parser.ParseFile(fset, "../annotations/annotations.go", nil, 0)
	...
	// Load the annotations listed as properties in PodPatcher
	p := PodPatcher{}
	pt := reflect.TypeOf(p)
