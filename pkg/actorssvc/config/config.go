...
// Config holds the configuration for the Actors service.
type Config struct {
	Port int
}

// FromConfigName returns an Actors configuration based on a configuration spec.
...
func getKubernetesConfig(configName string) (Config, error) {
	...
	list, err := daprClient.ConfigurationV1alpha1().
